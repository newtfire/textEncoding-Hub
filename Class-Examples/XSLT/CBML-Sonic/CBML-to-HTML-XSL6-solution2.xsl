...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!--   <xsl:variable name="cbml-collection" as="document-node()+" select="collection('cbml/?select=*.xml')"/>-->
   ...
   <xsl:template match="/">
            ...
            -->   
              ...
              <table>
                  ...
                  <xsl:for-each select="$myData">
                      ...
                      <tr>
                          <td class="item"><!-- This cell stores the current item() in the distinct-values variable! -->
                          ...
                          <td class="lookup">
                              ...
                              <!-- We're also going to pass a **tunneling parameter** to our next toc template:
                                  the value of the current() particular item in our table! 
                                  A tunneling parameter is like a variable, but we're using it to take some
    ...
    <!-- MODAL XSLT TEMPLATES!  -->
    <xsl:template match="div[@type='page']" mode="toc">
        <xsl:param name="currentItem"/>
        <!-- ebb: Here, just indicate the name of the tunneling parameter, so it knows where to come out and be processed! -->
        
        <!-- An xsl:if test tests if a condition is present, and ONLY outputs if the test passes as true.  -->
     
      <xsl:if test="descendant::cbml:panel/@characters[contains(., $currentItem)]"> 
           <h3>Page <xsl:value-of select="@xml:id ! substring-after(., '_')"/>
           <!--PARAM is <xsl:value-of select="$currentItem"/>-->
           </h3>
        <ul>
            <xsl:apply-templates select="descendant::cbml:panel" mode="toc">
                <xsl:with-param as="item()" name="currentItem" select="$currentItem"/>
            </xsl:apply-templates> 
            <!-- We still need to pass the parameter to the next TOC template so we can use it to 
            filter which panel links to ouptut. -->
        </ul>
      </xsl:if>
    ...
    <xsl:template match="cbml:panel" mode="toc">
        <xsl:param name="currentItem"/>
        <xsl:if test="contains(@characters, $currentItem)">
            <!-- Construct the links! -->
        ...
        <!-- END MODAL XSLT TEMPLATES! (REGULAR READING VIEW TEMPLATES FOLLOW) -->
